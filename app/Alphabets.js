...
import { View, Text, TouchableOpacity, StyleSheet, SafeAreaView, ImageBackground,
  Dimensions
 } from 'react-native';
...
export default function AlphabetScreen() {
  ...
  const checkIfDone = async () => {
    try {
      ...
      const res = await hasDone(kidId, flashScreen);
      const res2 = await hasDone(kidId, guessAlphabet);
      if (res?.isDone === 1) {
        setflashDone(1);
      }
      if (res2?.isDone === 1) {
  ...
  return (
    <ImageBackground source={require('../assets/images/mou.jpg')} style={styles.background}>
      <SafeAreaView style={styles.container}>
        <Link href="/learnandfun" style={styles.backButton}>
        ...
        <View style={styles.cardContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => router.push('/GuessAlphabet')}
          >
            <Text style={styles.cardTitle}>
              Play
              {findLetterDone === 1 ? (
